...
/// USB 标准设备请求(8 字节 SETUP 包的语义包装;发送点经 [`Self::raw`]
/// 取线格式)。
pub struct StdRequest([u8; 8]);

impl StdRequest {
    /// 线格式:8 字节 SETUP 包(`bmRequestType`/`bRequest`/`wValue`/`wIndex`/`wLength`)。
    #[inline]
    ...
    #[inline]
    pub fn get_descriptor_device() -> Self {
        Self([
