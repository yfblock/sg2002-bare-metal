...
register_bitfields![u32,
    ...
    /// ENA=1 写回去 = 禁用端口。掩码见 [`HPRT0::W1C_MASK`]。
...
/// RMW 写回 `HPRT0` 前**必须**屏蔽的 W1C 位（读 1 写回 = 误清/误禁用）——
/// 与 [`HPRT0`] 位域配套的寄存器级属性。需要显式清 `CONNDET` 时：
/// `hprt0.set((hprt0.get() & !HPRT0_W1C_MASK) | (1 << 1))`。
pub const HPRT0_W1C_MASK: u32 = (1 << 1) | (1 << 2) | (1 << 3) | (1 << 5);
