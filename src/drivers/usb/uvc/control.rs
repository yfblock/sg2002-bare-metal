...
fn pu_apply_one(ep: &dwc2::Ep0, vc_if: u8, pu: u8, bm: u32, ctrl: PuCtrl) {
    ...
    let want = try_get_def_u16(ep, vc_if, pu, ctrl.selector);
    match (cur, want) {
...
pub(crate) fn uvc_init_camera_controls(ep: &dwc2::Ep0, ent: &UvcControlEntities) -> UsbResult<()> {
    ...
    if let Some(pu) = ent.processing_unit_id {
        ...
        if (bm & (1 << 10)) != 0 {
            let _ = try_set_cur_u8(ep, vc_if, pu, PU_POWER_LINE_FREQUENCY_CONTROL, 1);
        }
    }
    ...
    if let Some(ct) = ent.camera_terminal_id {
        ...
        if (ent.ct_controls & (1 << 1)) != 0 {
            ...
            if (ent.ct_controls & (1 << 2)) != 0
                && try_set_cur_u8(ep, ent.vc_interface, ct, CT_AE_PRIORITY_CONTROL, 1)
            {
